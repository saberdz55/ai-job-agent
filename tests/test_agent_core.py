...
def test_agent_core_is_lazy_and_safe_without_sdk():
    ...
    assert "UNTRUSTED DATA" in core.SYSTEM
...
def test_model_output_fails_closed_on_secret_markers():
    from job_agent.agent_core import validate_model_output
    assert validate_model_output("hello") == "hello"
    assert "cannot provide secrets" in validate_model_output("OPENAI_API_KEY=secret")
    assert "cannot provide secrets" in validate_model_output("Authorization: Bearer abc")
